...
class Error_Of_Trail:
    def __init__(self,trail,error_type,error_value,error_position,length_parameter,SPT_cable_length,r1=1,r2=1):
        ...
        self.tuning_parameters=jg.tuning_zone_parameters(
            ...
            SVA_type=find_SVA_type(self.error_position)
        )
    def character(self):
    ...
    def call_matrix_main(self):
        ...
        if self.error_type==0 or self.error_type==3 or self.error_type==4 or self.error_type==1:
            ...
            try:
                ...
                # 提取电流值并保存到对应的变量中
                if isinstance(self.output_voltage_surface1, np.ndarray) and self.output_voltage_surface1.shape == (2, 1):
                    self.output_current_surface1 = self.output_voltage_surface1[1][0]
                if isinstance(self.output_voltage_surface2, np.ndarray) and self.output_voltage_surface2.shape == (2, 1):
                    self.output_current_surface2 = self.output_voltage_surface2[1][0]
                if isinstance(self.output_voltage_main, np.ndarray) and self.output_voltage_main.shape == (2, 1):
                    self.output_current_main = self.output_voltage_main[1][0]
            except Exception as e:
    ...
    def build_circuit_model(self):
        ...
        try:
            ...
            # 10. 计算调谐区参数
            BA_types = self.find_BA_type_tuning_zone()
            tuning_params = jg.tuning_zone_parameters(variable, self.length_parameter, BA_types[0], BA_types[1], find_SVA_type(self.error_position))
